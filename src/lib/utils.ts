...
/**
 * Trigger a browser download of an audio (or any) URL.
 * Tries blob-fetch first (works cross-origin when CORS allows it),
 * then falls back to an anchor click using the `download` attribute.
 */
export async function downloadAudioFile(url: string, filename: string): Promise<void> {
  try {
    ...
    // Fallback: open in a new tab (browser may still download based on headers)
    const a = document.createElement("a");
    a.href = url;
    a.download = filename;
    a.target = "_blank";
